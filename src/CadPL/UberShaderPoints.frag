...
void main()
{
	...
	if(textureOffset == 0)
		...
		textureInfo = TextureInfoRef(inDrawableDataPtr + textureOffset);
	...
	if(textureType == 0x0100) {
		...
		vec2 uv = computeTextureCoordinates(textureInfo, inVertexDataPtr);
	...
	// implemented in the similar was as OpenGL does
		...
		// material data
		PhongMaterialRef phongMaterial = PhongMaterialRef(inDrawableDataPtr);
		...
		if(getMaterialUseColorAttribute()) {
			uint colorAccessInfo = getColorAccessInfo();
			if(getMaterialIgnoreColorAttributeAlpha()) {
				diffuseColor = readVec3(inVertexDataPtr, colorAccessInfo);
				if(getMaterialIgnoreMaterialAlpha())
					outColor.a = 1;
				else
					outColor.a *= phongMaterial.diffuseAndAlpha.a;
			} else {
				vec4 color = readVec4(inVertexDataPtr, colorAccessInfo);
	...
	// implemented in the similar way as glTF does
		...
		if(lightSettings != 0) {
			...
			// material data
			MetallicRoughnessMaterialRef metallicRoughnessMaterial = MetallicRoughnessMaterialRef(inDrawableDataPtr);
