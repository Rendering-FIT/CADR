...
inline PipelineSceneGraph::PipelineSceneGraph(CadR::StateSet& root, const std::vector<std::bitset<ShaderState::numOptimizeFlags>>& optimizationLevels)  : PipelineSceneGraph(nullptr, root, optimizationLevels) { _shaderLibrary=new ShaderLibrary(root.renderer().device()); _pipelineLibrary=new PipelineLibrary(*_shaderLibrary); }
...
PipelineSceneGraph::~PipelineSceneGraph() noexcept  { _stateSetMap.clear_and_dispose([](StateSetMapItem* item){ delete item; }); if(_deleteLibraries) { delete _pipelineLibrary; delete _shaderLibrary; } }
