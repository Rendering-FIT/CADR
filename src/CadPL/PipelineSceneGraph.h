...
inline void PipelineSceneGraph::init(CadR::StateSet& root, const std::vector<std::bitset<ShaderState::numOptimizeFlags>>& optimizationLevels, uint32_t maxTextures)  { destroy(); _root=&root; _shaderLibrary=new ShaderLibrary(root.renderer().device()); _pipelineLibrary=new PipelineLibrary(*_shaderLibrary); }
inline void PipelineSceneGraph::destroy() noexcept  { _stateSetMap.clear_and_dispose([](StateSetMapItem* item){ delete item; }); if(_deleteLibraries) { delete _pipelineLibrary; _pipelineLibrary= nullptr; delete _shaderLibrary; _shaderLibrary=nullptr; } }
